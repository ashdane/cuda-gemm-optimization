...
K6 Warptile & 3515.8 $\pm$ 356.9 & 27.35 $\pm$ 2.15 & 26.1\% & 2.3x & N/A \\
...
\begin{figure}[h]
\centering
\includegraphics[width=1.00\textwidth]{kernel_progression.png}
\caption{Kernel speeds on Pascal vs Turing.}
\end{figure}
...
The heatmap in Figure 2 directly visualizes this cliff across the parameter sweep grid ($M=N=K=4096$). Moving from $TN=4$ to $TN=8$ cuts performance by 35\% to 50\% across every single combination of $BM$ and $BN$. At $BM=128, BN=128$, the throughput plummets from 8084.9 GFLOPS down to 5210.3 GFLOPS. At $BM=64, BN=128$, it drops from 7560.0 to 4719.1 GFLOPS.
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{dimension_sweep.png}
\caption{Square size scaling.}
\end{figure}
...
\begin{tabular}{|l|r|r|r|r|r|}
\hline
\textbf{Kernel} & \textbf{Regs} & \textbf{Smem} & \textbf{Threads} & \textbf{Pascal Occ.} & \textbf{Turing Occ.} \\
\hline
K0 Naive & 32 & 0 B & 1024 & 100\% & 100\% \\
K2 Smem & 30 & 8 KB & 1024 & 100\% & 100\% \\
K4 2D Block & 96 & 8 KB & 256 & 25\% & 50\% \\
K5 Vectorized & 92 & 16 KB & 256 & 25\% & 50\% \\
K6 Warptile & 64 & 16 KB & 256 & 37.5\% & 100\% \\
K7 DoubleBuf & 112 & 32 KB & 256 & 12.5\% & 50\% \\
\hline
\end{tabular}
\caption{Registers and theoretical occupancy from ptxas.}
\end{table}
...
\subsection{Split-K Decomposition for Tall/Skinny GEMM}
When $M$ and $N$ are small but $K$ is very large (common in attention projections and batch-1 autoregressive inference), standard 2D grid tiling $(M/BM, N/BN)$ yields too few threadblocks to saturate GPU SMs. For example, on a $128 \times 128 \times 65{,}536$ problem with standard $128 \times 128$ block tiles, the launch grid is $(1, 1)$, creating \textbf{exactly 1 threadblock} and leaving 67 out of 68 SMs completely idle on Turing.

To test this empirically, I implemented and benchmarked a Split-K variation that divides the 65,536 reduction dimension into $S=64$ equal slices across threadblocks, accumulating partial products into a temporary workspace before running a lightweight reduction pass. Table 11 shows the measured comparison on the Turing RTX 2080 Ti:

\begin{table}[h]
\centering
\begin{tabular}{|l|r|r|r|l|}
\hline
\textbf{Kernel Strategy} & \textbf{Active Blocks} & \textbf{Time (ms)} & \textbf{Throughput (GFLOPS)} & \textbf{SM Utilization} \\
\hline
Standard 2D Blocktile (K4) & 1 & 2.164 $\pm$ 0.041 & 992.5 $\pm$ 18.8 & Severe under-subscription (1/68 SMs) \\
Split-K ($S=64$ slices) & 64 & 0.448 $\pm$ 0.015 & 4794.2 $\pm$ 160.5 & High wave saturation (64/68 SMs) \\
cuBLAS Reference & Adaptive & 0.222 $\pm$ 0.008 & 9674.8 $\pm$ 348.6 & Vendor autotuned Split-K \\
\hline
\end{tabular}
\caption{Empirical evaluation of Split-K on a tall/skinny matrix ($128 \times 128 \times 65{,}536$) on Turing (median $\pm$ stddev over 100 timed runs).}
\end{table}

The experimental results validate the theoretical motivation:
\begin{itemize}
    \item On standard 2D block tiling, throughput collapses to \textbf{992.5 GFLOPS} because 67 out of 68 SMs are idle.
    \item Split-K distributes the reduction loop across 64 threadblocks, saturating nearly all SMs and achieving \textbf{4794.2 GFLOPS}—a \textbf{4.83x empirical speedup} over standard 2D tiling on the exact same problem dimensions.
    \item While the secondary reduction phase introduces a modest memory-write overhead, the dramatic increase in SM wave quantization and thread-level concurrency vastly outweighs the reduction cost on skinny shapes.
\end{itemize}

\subsection{Mixed-Precision and Quantized GEMM (INT8 / DP4A)}
For inference workloads, full FP32 precision is frequently unnecessary.
\begin{itemize}
    \item \textbf{Pascal DP4A:} On sm\_61, the \texttt{\_\_dp4a} intrinsic performs a 4-element 8-bit integer dot product with 32-bit accumulation in a single instruction, effectively delivering 4x the theoretical arithmetic throughput of standard FP32 instructions.
    \item \textbf{Turing Tensor Cores:} Turing provides hardware INT8/INT4 Tensor Cores alongside FP16 WMMA, delivering over 200 TOPS of compute. Moving to lower precisions shifts memory bandwidth bottlenecks, allowing memory-bound regimes to achieve much higher effective compute efficiency.
\end{itemize}
