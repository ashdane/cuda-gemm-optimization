...
\subsection{Split-K Decomposition for Tall/Skinny GEMM (Theoretical Analysis)}
...
While I did not implement a custom Split-K kernel for this report, the theoretical motivation for Split-K is clear from our occupancy analysis:
\begin{itemize}
    \item \textbf{Under-subscription penalty:} Standard 2D block tiling limits concurrency to the $M$ and $N$ dimensions. When computing a single $128 \times 128$ output tile, the entire 65,536-element reduction loop is serialized within a single threadblock, stranding the vast majority of the GPU's arithmetic pipelines.
    \item \textbf{Split-K scaling:} A Split-K algorithm addresses this by dividing the $K$ dimension into $S$ equal slices (e.g., $S=64$). Each slice is computed independently by a separate threadblock, outputting partial results to a temporary global workspace. A final lightweight reduction pass then accumulates these partials.
    \item \textbf{Wave quantization tradeoff:} By launching $1 \times 64 = 64$ active threadblocks instead of 1, Split-K allows the GPU to achieve near-perfect wave saturation across the 68 SMs on Turing. While the secondary reduction phase introduces a modest global memory-write overhead, the massive increase in thread-level concurrency theoretically enables throughput to scale almost linearly with $S$ (up to the SM count) compared to the severe bottleneck of a single active SM.
\end{itemize}
