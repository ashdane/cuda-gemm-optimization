...
Note on K6 (Warptile) and automated sweeps: In the progression benchmark (Table 3), K6 was run with its canonical configuration ($BM=128, BN=128, BK=16, WM=64, WN=32, TM=16, TN=4$, 256 threads/block), yielding $3,515.8 \pm 356.9$ GFLOPS. Later, in the 144-run parameter sweep (detailed in §4.1), configurations with $\ge 512$ threads/block (e.g. 1024 threads with $TM=4, TN=4$) produced physically impossible GFLOPS readings over 17,000 in the CSV. Rather than a silent truncation, this is hypothesized to be a profiling artifact caused by an out-of-bounds memory access: assigning too many warps to a fixed $128 \times 128$ tile pushes thread indices beyond the allocated bounds. As seen elsewhere in this report, such OOB access triggers a \texttt{cudaErrorIllegalAddress}. If the benchmarking wrapper fails to properly synchronize and catch the fault, the asynchronous \texttt{cudaEventRecord} resolves almost instantly (measuring only launch overhead), which the script blindly calculates as $>17,000$ GFLOPS. The canonical K6 implementation achieving valid results is the $3515.8$ GFLOPS reported above.
...
Beyond standard dense tiled GEMM, the assignment encouraged experimenting with alternative ways to multiply matrices. Since theoretical sub-cubic algorithms like Strassen perform poorly on GPUs due to $\mathcal{O}(N^2)$ memory allocation bandwidth overheads, I focused my empirical alternative experimentation on hardware-accelerated mixed-precision math.

\subsection{Mixed-Precision GEMM via Tensor Cores (Empirical)}
To test an alternative mathematical formulation of GEMM, I implemented \textbf{Kernel 8 (K8)}, which abandons standard CUDA core fused multiply-adds (FMA) in favor of the Warp Matrix Multiply and Accumulate (WMMA) API. This algorithm computes $16 \times 16 \times 16$ block matrix multiplications natively in hardware.

\begin{itemize}
    \item \textbf{Implementation Details:} The \texttt{08\_tensor\_core\_wmma.cu} kernel casts the FP32 input arrays into FP16 precision using \texttt{\_\_float2half}, streams them into \texttt{wmma::fragment} registers, and accumulates the result in FP32. This represents a fundamental shift from scalar threaded math to warp-level hardware matrix math.
    \item \textbf{Empirical Performance:} As shown in Table 3, the alternative K8 Tensor Core approach achieved \textbf{6779.5 GFLOPS} at $4096^3$. While this is $4.4\times$ faster than the naive baseline, it fell drastically short of the peak 107.6 TFLOPS Tensor Core ceiling (reaching only 6.3\% efficiency). 
    \item \textbf{Architectural Takeaway:} The profiling confirms that simply switching to an alternative Tensor Core algorithm is insufficient without an equally advanced memory hierarchy strategy. Without the complex shared memory staging and asynchronous data prefetching utilized by cuBLAS (which achieved 34,233 GFLOPS using Tensor Cores), the sheer arithmetic throughput of the WMMA pipeline instantly starves the L1 cache, shifting the algorithm from compute-bound back to severely memory-bound.
\end{itemize}

While I originally investigated theoretical limits for INT8/DP4A quantized execution and Strassen's algorithm, I restricted the reported analysis here strictly to the WMMA Tensor Core approach, as it is the only alternative algorithmic pipeline I could rigorously test and profile natively on the Turing hardware.
